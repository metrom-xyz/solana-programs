...
describe("Transfer campaign ownership", () => {
    ...
    it("Fails when forbidden", async () => {
        ...
        await program.methods
            .transferCampaignOwnership(campaign.id)
            .accounts({
    ...
    it("Succeeds", async () => {
        ...
        expect(campaignPost.base.owner).to.be.deep.equal(
            program.provider.wallet.publicKey
        );
        expect(campaignPost.base.pendingOwner).to.be.deep.equal(
            newOwner.publicKey
        );
    });
});
